package com.orangemust.love.swagger.core;
...
import com.orangemust.love.swagger.annotation.ApiGroupProperties;
import com.orangemust.love.swagger.annotation.ApiGroupProperty;
import com.orangemust.love.swagger.bean.BaseModel;
import com.orangemust.love.swagger.util.ReflectUtils;
...
public class ApiGroupModelReader extends ApiModelReader {
    ...
    @Autowired
    public ApiGroupModelReader(@Qualifier("cachedModels") ModelProvider modelProvider, TypeResolver typeResolver,
        ...
        modelMap.entrySet().forEach(entry -> {
                    ...
                    for (ApiGroupProperty apiGroupProperty : apiGroupPropertyArray) {
                        ...
                        if (apiGroupProperty != null) {
                            ...
                            for (Class<?> groupClazz : apiGroupProperty.value()) {
                                String groupName = org.apache.commons.lang.StringUtils
                                        .substringAfterLast(groupClazz.getName(), ".");
                                ...
                                if (model == null) {
                                    ...
                                    model = new Model(id, item.getName(), item.getType(), item.getQualifiedType(),
                                            properties, item.getDescription(),
                                            item.getBaseModel(), item.getDiscriminator(), item.getSubTypes(),
                                            item.getExample(), item.getXml());
            ...
            if (context.getReturnType().equals(item.getType())) {
                ...
                if (annotation.isPresent()) {
                    ...
                    Model model = new Model(id, item.getName(), item.getType(), item.getQualifiedType(), properties,
                            item.getDescription(),
                            item.getBaseModel(), item.getDiscriminator(), item.getSubTypes(), item.getExample(),
                            item.getXml());
                    currentModel.put(id, model);
                    CACHE.put(id, model);
    ...
    private ModelProperty generateModelProperty(ModelProperty modelProperty, String groupName,
            ApiGroupProperty apiGroupProperty) {
        ModelProperty currentModelProperty = generateModelProperty(modelProperty, groupName);
        ...
        if (StringUtils.isNotBlank(apiGroupProperty.description())) {
            ...
            currentModelProperty = new ModelProperty(modelProperty.getName(), type,
                    ...
                    modelProperty.isHidden(), modelProperty.isReadOnly(), modelProperty.isAllowEmptyValue(),
                    description,
