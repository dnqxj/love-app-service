...
public class Album {
    ...
    public Result list(HttpServletRequest request) {
        ...
        try {
            List list = D.M(AlbumModel.class).field("t_album.*, t_resources.url_path as urlPath")
                    .join("left join t_resources on resources_uuid=t_resources.uuid").where("t_album.uid=?", userId)
                    .select();
    ...
    @PostMapping("add")
    public Result add(
            @ApiGroup(AlbumGroups.Insert.class) @Validated(AlbumGroups.Insert.class) @RequestBody AlbumModel albumModel,
    ...
    @PostMapping("/delete")
    public Result delte(
            @ApiGroup(AlbumGroups.Delete.class) @Validated(AlbumGroups.Delete.class) @RequestBody AlbumModel albumModel,
            HttpServletRequest request) {
        Long uid = FuncUtil.getUid(request);
        // 检查该资源uuid是否真实存在
